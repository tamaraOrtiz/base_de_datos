...
SELECT t1.id, t1.fecha_vencimiento, t1.cliente, t2.nombre, t3.saldo FROM Venta_facturas t1, Clientes t2, Venta_detalles t3
	WHERE ((t1.fecha_vencimiento < CURDATE() AND t3.saldo > 0) AND (t1.cliente = t2.id AND t3.venta_id = t1.id));
